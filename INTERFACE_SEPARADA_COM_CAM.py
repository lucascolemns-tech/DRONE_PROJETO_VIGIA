...
class ESPCom:
    def __init__(self, ip="10.159.79.50", porta=1244):
        self.ip = ip
        self.porta = porta
        self.sock = None
        self.buffer = ""  
...
def thread_controle():
    ...
    caminho_exe = r"C:\Users\nsamu\Downloads\META_DRONE - Copia\INTERFACE_PYTHON\controle.exe"
    try:
...
def permissao3():
    camera_janela = ctk.CTkToplevel(window)
  
    url = "http://192.168.1.19:81/stream"
    cap = cv2.VideoCapture(url)
    imagem_label = ctk.CTkLabel(camera_janela, text="")
    imagem_label.pack()

    def atualizar_camera():
        ret, frame = cap.read()
        if ret:
            largura = camera_janela.winfo_width()
            altura = camera_janela.winfo_height()
        ...
        camera_janela.after(10, atualizar_camera)
    atualizar_camera()
...
btn5 = ctk.CTkButton(master=abas, fg_color="#FFFFFF", border_width=3, border_color="#292525", hover_color="#D1D5DB", text_color="black", text="exibir camera", corner_radius=6, height=60, font=("Arial Black", 20), state="normal", command=permissao3)
